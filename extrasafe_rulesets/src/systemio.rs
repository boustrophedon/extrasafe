...
use {
    extrasafe::{seccomp_arg_filter, YesReally, syscalls::Sysno, RuleSet, SeccompRule},
    std::{
...
use {
    extrasafe::{access, AccessFs, BitFlags, LandlockRule},
...
impl SystemIO {
    ...
    pub fn allow_open_readonly(mut self) -> SystemIO {
        ...
        self.custom.entry(Sysno::open).or_default().push(rule);

        let rule = SeccompRule::new(Sysno::openat)
            .and_condition(seccomp_arg_filter!(arg2 & WRITECREATE == 0));
        self.custom.entry(Sysno::openat).or_default().push(rule);
    ...
    pub fn allow_stdin(mut self) -> SystemIO {
        ...
        self.custom.entry(Sysno::read).or_default().push(rule);
    ...
    pub fn allow_stdout(mut self) -> SystemIO {
        ...
        self.custom.entry(Sysno::write).or_default().push(rule);
    ...
    pub fn allow_file_read(mut self, file: &File) -> SystemIO {
        ...
        for &syscall in IO_READ_SYSCALLS {
            ...
            self.custom.entry(syscall).or_default().push(rule);
        }
