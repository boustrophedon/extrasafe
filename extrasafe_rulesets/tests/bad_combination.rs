use {
    extrasafe::{syscalls::Sysno, RuleSet, SafetyContext, SeccompRule},
...
fn invalid_combination_read_and_stdin() {
    let res = SafetyContext::new().enable(SystemIO::nothing().allow_read().allow_stdin());
    assert!(
        res.is_err(),
        "Extrasafe didn't fail when adding conflicting rules"
    );
...
fn not_invalid_combination_duplicate_simple() {
    let res = SafetyContext::new().enable(SystemIO::nothing().allow_read().allow_read());
    assert!(res.is_ok());
...
fn not_invalid_combination_duplicate_conditional() {
    let res = SafetyContext::new().enable(SystemIO::nothing().allow_stdin().allow_stdin());
    assert!(res.is_ok());
