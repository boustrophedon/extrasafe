...
use {
    ...
    extrasafe_rulesets::{danger_zone::Threads, Networking},
    std::{
        io::{Read, Write},
...
fn test_udp() {
    ...
    let server_handle = spawn(move || {
        let server_socket = UdpSocket::bind("127.0.0.1:30357").unwrap();
    ...
    let client_socket = UdpSocket::bind("127.0.0.1:30493").unwrap();
    ...
    let res = UdpSocket::bind("127.0.0.1:30358");
...
fn test_tcp() {
    ...
    let server_handle = spawn(move || {
        let server_socket = TcpListener::bind("127.0.0.1:31357").unwrap();
    ...
    let mut client_socket = TcpStream::connect("127.0.0.1:31357").unwrap();
    ...
    // now try to bind again and fail
    let res = TcpListener::bind("127.0.0.1:31359");
    assert!(res.is_err(), "Incorrectly succeeded in binding to socket");
}
...
fn test_start_tcp() {
    SafetyContext::new()
        ...
        .apply_to_current_thread()
        .unwrap();
    let tcp_res = TcpListener::bind("127.0.0.1:0");
    assert!(tcp_res.is_ok(), "Failed to bind tcp server");

    let udp_res = UdpSocket::bind("127.0.0.1:0");
    assert!(
        udp_res.is_err(),
    ...
    // test ipv6 as well
    let tcp_res = TcpListener::bind("[::1]:0");
    assert!(tcp_res.is_ok(), "Failed to bind tcp server");

    let udp_res = UdpSocket::bind("[::1]:0");
    assert!(
        udp_res.is_err(),
        "Incorrectly succeeded in binding udp socket"
    );
...
fn test_start_udp() {
    ...
    let udp_res = TcpListener::bind("127.0.0.1:0");
    assert!(
        udp_res.is_err(),
        "Incorrectly succeeded in binding udp socket"
    );
...
fn test_start_unix() {
    ...
    let unix_res = UnixListener::bind(&path);
