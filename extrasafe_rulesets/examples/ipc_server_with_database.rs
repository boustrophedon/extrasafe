...
fn run_subprocess(cmd: &[&str]) -> Child {
...
fn run_webserver(db_socket_path: &str, our_socket_path: &str) {
    ...
    socket
        .connect(db_socket_path)
        .expect("failed to connect to db socket");
    ...
    let runtime = RuntimeBuilder::new_current_thread()
        ...
        .build()
        .unwrap();
    let listener = TcpListener::bind("127.0.0.1:5576").unwrap();
    ...
    let routes = warp::path("write")
        ...
        .map(|param: Bytes, db_conn: DbConn| {
            println!("webserver got write request");
            let conn = db_conn.lock().unwrap();

            let s = from_utf8(&param).unwrap();
