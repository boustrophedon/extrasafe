...
mod with_isolate {
    use {
        extrasafe::{isolate::Isolate, SafetyContext},
    ...
    fn check_isolate_output(isolate_name: &'static str, data: &[&str], envs: &HashMap<String, String>) {
        ...
        for path in read_dir("/tmp").unwrap().flatten() {
            let path = path.path();
    ...
    fn test_multiple_binds() {
        ...
        let f1 = read_to_string(path1.join("hello")).unwrap();
        let f2 = read_to_string(path2.join("hey")).unwrap();
    ...
    /// Test we can use a standard [`SafetyContext`] inside an `Isolate`
    ...
    fn check_mountinfo() {
        ...
        let s = read_to_string("/proc/self/mountinfo").unwrap();
        println!("mountinfo:\n{}", s);
    }
    ...
    fn isolate_unix_socket(name: &'static str) -> Isolate {
        ...
        let path = env::var("ISOLATE_SOCKET_PATH").unwrap();
    ...
    fn isolate_multiple_binds(name: &'static str) -> Isolate {
        ...
        let path1 = env::var("ISOLATE_DIR_1").unwrap();
        let path1 = PathBuf::from(path1);
        let path2 = env::var("ISOLATE_DIR_2").unwrap();
