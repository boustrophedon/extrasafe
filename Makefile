...
test:
	...
	cargo run --all-features --example isolate_drop_capabilities
...
test-ci:
	...
	cargo run --target=$(TARGET_TRIPLE) --all-features --example isolate_drop_capabilities
...
do-cov:
	...
	cargo llvm-cov --no-report --all-features run --example isolate_drop_capabilities
